...
void l_fire_station()
{
  ...
  print2("Enter the flames? [Y|N] ");
  if (ynq2()=='y') {
    if (Player.hp == 1) p_death("total incineration");
    else Player.hp = 1;
    dataprint();
    print1("You feel like you are being incinerated! Jump back? [Y|N] ");
    if (ynq1()=='y')
...
void l_water_station()
{
  ...
  if (ynq1()=='y') {
    ...
    morewait();
    print2("Your body begins to disintegrate.... Leave the pool? [Y|N] ");
    if (ynq2()=='y')
      print2("Phew! That was close!");
    else {
      clearmsg();
...
void l_void_station()
{
  ...
  print1("You are at the brink of an endless void. Enter it? [Y|N] ");
  if (ynq()=='y') {
    if (Level->mlist == NULL) {
...
void l_enter_court()
{
  print1("You have found a magical portal! Enter it? [Y|N] ");
  if (ynq1()=='y') {
...
void l_balancestone()
{
  ...
  if (ynq1()=='y') {
	...
	Player.y = random_range(LENGTH);
      ...
      print2("You are being drained of experience! Step back? [Y|N] ");
      if (ynq2()=='y') {
	clearmsg();
...
void l_sacrificestone()
{
  ...
  print2("You sense something awakening. Touch the block? [Y|N] ");
  if (ynq2() == 'y') {
    print1("You sense great pain emanating from the ancient altar.");
    print2("Climb on to the block? [Y|N] ");
    if (ynq2() == 'y') {
...
void l_mindstone()
{
  ...
  print2("Look away from the interesting phenomenon? [Y|N] ");
  if (ynq2()=='n') {

...
struct monster *m;
{
  ...
  if (! m_statusp(m,HOSTILE)) {
    ...
    mprint("Do you request a ritual of neutralization? [Y|N] ");
    if (ynq() == 'y') {
    ...
    mprint("'Have you learned your lesson?' The ArchDruid asks. [Y|N] ");
    if (ynq()) {
...
struct monster *m;
{
  if (m_statusp(m,HOSTILE)) 
    ...
    mprint("The horse lets you pat his nose. Want to ride him? [Y|N] ");
    if (ynq()=='y') {
      m->hp = -1;
...
struct monster *m;
{
  ...
  if (m->id == SERV_LAW) {
    ...
    mprint("You are asked: Are there any Servants of Chaos hereabouts? [Y|N] ");
  }
  else {
    target = SERV_LAW;
    mprint("The Servant of Chaos grins mischievously at you.");
    mprint("You are asked: Are there any Servants of Law hereabouts? [Y|N] ");
  }
  if (ynq()=='y') {
    print1("Show me.");
    show_screen();

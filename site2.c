...
void l_condo()
{
  ...
  if (! gamestatusp(SOLD_CONDO)) {
    ...
    response = mgetc();
    if (response == 'p') {
      print2("Only 50,000Au. Buy it? [Y|N] ");
      if (ynq2()=='y') {
	if (Player.cash < 50000) 
      ...
      print2("Weekly Rental, 1000Au. Pay for it? [Y|N] ");
      if (ynq2()=='y') {
	if (Player.cash < 1000)
	...
	clearmsg();
	print1("You sure you want to retire, now? [Y|N] ");
	if (ynq1() == 'y') {
	  p_win();
	}
      }
...
void l_adept()
{
  ...
  print2("Enter the mystic portal? [Y|N] ");
  if (ynq2()!='y') {
    if (Player.level > 100) {
...
void l_brothel()
{
  ...
  if (ynq1()=='y') {
	...
	print2("`500Au, buddy. For the night.' pay it? [Y|N] ");
	if (ynq2()=='y') {
	  if (Player.cash < 500) {
	    print1("`What, no roll?!'");
...
void l_oracle()
{
  ...
  if (gamestatusp(ATTACKED_ORACLE) && (! gamestatusp(COMPLETED_ASTRAL))) {
	...
	if (!gamestatusp(SPOKE_TO_DRUID)) {
	  ...
	  print2("The oracle holds out her hand. Do you take it? [Y|N] ");
	  if (ynq2()=='y') {
...
void l_cartographer()
{
  ...
  print2("Map of the local area: 500Au. Buy it? [Y|N] ");
  if (ynq2()=='y') {
    if (Player.cash < 500) 
...
{
  long donation;
  print2("'Greetings, friend. Do you wish to make a donation?' [Y|N] ");
  if (ynq2()!='y') 
    print3("'Pinchpurse!'");
  else {
    clearmsg();

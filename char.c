...
FILE *omegarc_check()
{
  ...
  if ((fd = fopen("omega.rc","rb")) != NULL) {
    print2("Use omega.rc charcter record in current directory? [Y|N] ");
#else
  sprintf(Str1, "%s/.omegarc", getenv("HOME"));
  if ((fd = fopen(Str1,"r")) != NULL) {
    print2("Use .omegarc in home directory? [Y|N] ");
#endif
    if (ynq2()!='y') {
      fclose(fd);
      fd = NULL;
...
void initstats()
{
  ...
  if (response == 'c') omegan_character_stats();
    ...
    user_intro();
#if defined(MSDOS) || defined(AMIGA)
    print1("Do you want to save this set-up to omega.rc in this directory? [Y|N] ");
#else
    print1("Do you want to save this set-up to .omegarc in your home directory? [Y|N] ");
#endif
    if (ynq1()=='y')
      save_omegarc();
  }
...
void user_character_stats()
{
  ...
  print1("Took an official IQ test? [Y|N] ");
  if (ynq1()=='y') {
  ...
  print1("Took Undergraduate entrance exams? [Y|N] ");
  if (ynq1()=='y') {
    do {
  ...
  print1("Took Graduate entrance exams? [Y|N] ");
  if (ynq1()=='y') {
    do {
      print1("So, what percentile? ");
      num = (int) parsenum();
  ...
  if (numints == 0) {
    print1("Pretty dumb, aren't you? [Y|N] ");
    if (ynq1()=='y') {
  ...
  agipts = 0;
  print1("Can you dance? [Y|N] ");
  if (ynq1()=='y') {
    agipts++;
    nprint1(" Well? [Y|N] ");
    if (ynq1()=='y') agipts+=2;
  }
  print1("Do you have training in a martial art or gymnastics? [Y|N] ");
  if (ynq1()=='y') {
    agipts+=2;
    print2("Do you have dan rank or equivalent? [Y|N] ");
    if (ynq2()=='y') agipts+=4;
  }
  clearmsg();
  print1("Do you play some field sport? [Y|N] ");
  if (ynq1()=='y') {
    agipts++;
    nprint1(" Are you good? [Y|N] ");
    if (ynq1()=='y') agipts++;
  }
  print1("Do you cave, mountaineer, etc.? [Y|N] ");
  if (ynq1()=='y')
    agipts+=3;
  print1("Do you skate or ski? [Y|N] ");
  if (ynq1()=='y') {
    agipts+=2;
    nprint1(" Well? [Y|N] ");
    if (ynq1()=='y') agipts+=2;
  }
  print1("Are you physically handicapped? [Y|N] ");
  if (ynq1()=='y')
    agipts-=4;
  print1("Are you accident prone? [Y|N] ");
  if (ynq1()=='y')
    agipts-=4;
  print1("Can you use a bicycle? [Y|N] ");
  if (ynq1()!='y')
    agipts-=4;
  Player.agi = Player.maxagi = 9 + agipts/2;
  print1("Do you play video games? [Y|N] ");
  if (ynq1()=='y') {
    dexpts+=2;
    print2("Do you get high scores? [Y|N] ");
    if (ynq2()=='y') dexpts+=4;
  }
  clearmsg();
  print1("Are you an archer, fencer, or marksman? [Y|N] ");
  if (ynq1()=='y') {
    dexpts+=2;
    print2("A good one? [Y|N] ");
    if (ynq2()=='y') dexpts+=4;
  }
  clearmsg();
  print1("Have you ever picked a lock? [Y|N] ");
  if (ynq1()=='y') {
    dexpts+=2;
  ...
  dexpts += num/25;
  print1("Hold your arm out. Tense your fist. Hand shaking? [Y|N] ");
  if (ynq1()=='y')
    dexpts-=3;
  print1("Ambidextrous, are you? [Y|N] ");
  if (ynq1()=='y')
    dexpts+=4;
  print1("Can you cut a deck of cards with one hand? [Y|N] ");
  if (ynq1()=='y')
    dexpts+=2;
  print1("Can you tie your shoes blindfolded? [Y|N] ");
  if (ynq1()!='y')
    dexpts-=3;
  Player.dex = Player.maxdex = 6 + dexpts/2;
  print1("Do you ever get colds? [Y|N] ");
  if (ynq1()!='y') 
    conpts+=4;
  else {
    nprint1(" Frequently? [Y|N] ");
    if (ynq1() == 'y') conpts -=4;
  }
  print1("Had any serious accident or illness this year? [Y|N] ");
  if (ynq1()=='y') conpts -=4;
  else conpts +=4;
  print1("Have a chronic disease? [Y|N] ");
  if (ynq1() =='y') conpts -=4;
  print1("Overweight or underweight by more than 20 percent? [Y|N] ");
  if (ynq1() =='y') conpts -=2;
  print1("High Blood Pressure? [Y|N] ");
  if (ynq1() =='y') conpts -=2;
  print1("Smoke? [Y|N] ");
  if (ynq1() =='y') conpts -=3;
  print1("Take aerobics classes? [Y|N] ");
  if (ynq1() =='y') conpts +=2;
  ...
  print1("Do animals react oddly to your presence? [Y|N] ");
  if (ynq1()=='y') {
  ...
  print1("Can you see auras? [Y|N] ");
  if (ynq1()=='y') {
    nprint1(" How strange.");
    morewait();
    powpts += 3;
  }
  print1("Ever have an out-of-body experience? [Y|N] ");
  if (ynq1()=='y') {
  ...
  print1("Did you ever cast a spell? [Y|N] ");
  if (ynq1()=='y') {
    powpts += 3;
    nprint1(" Did it work? [Y|N] ");
    if (ynq1()=='y') {
      powpts+=7;
      print2("Sure it did.");
      morewait();
  ...
  print1("Do you believe in ghosts? [Y|N] ");
  if (ynq1()=='y') {
    powpts+=2;

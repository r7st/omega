...
#define hinthour club_hinthour
  ...
  if (! gamestatusp(CLUB_MEMBER)) {
    if (Player.level < 2) print3("Only reknowned adventurers need apply.");
      ...
      print2("Dues are 100Au. Pay it? [Y|N] ");
      if (ynq2()=='y') {
	if (Player.cash < 100)
...
void l_commandant()
{
  ...
  print2("Buy a bucket! Only 5 Au. Make a purchase? [Y|N] ");
  if (ynq2()=='y') {
    clearmsg();
    print1("How many? ");
    num = (int) parsenum();
...
void l_crap()
{
  ...
  if ((hour() < 17) || (hour() > 23))
    ...
    print2("May I take your order? [Y|N] ");
    if (ynq2()=='y') {
      if (Player.cash < 1000)
...
void l_alchemist()
{
  ...
  if (nighttime()) 
	...
	if (Monsters[obj->charge].transformid == -1)
	  ...
	  nprint1("Au for the transformation. Pay it? [Y|N] ");
	  if (ynq1()=='y') {
...
void l_dpw()
{
  ...
  if (Date - LastDay < 7)
    ...
    print2("Do you want to go on the dole? [Y|N] ");
    if (ynq2()=='y') {
